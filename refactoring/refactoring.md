...
#### Large Scale Refactoring - aka rewriting, replatforming, re-engineering
...
* When refactoring, we often support parts of the old and new system at the same time; whereas, when rewriting, we usually ﬂip a switch to move from the old system to the new. This makes us less aggressive in putting the new system in place, so that we must live with the old system (and its problems) in its entirety for longer.

Before you start a rewrite, you need to understand and document all the business logic in the application you intend to rewrite. Generally, with a legacy application, you have neither understanding nor documentation.

#### Slightly different definition

The act of identifying a specifiic problem with code, envisioning how to improve it, then improving it in small, reversible steps
